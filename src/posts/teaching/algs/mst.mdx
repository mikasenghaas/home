...
### Trace of Prim's, Kruskal's and Dijkstra's
...
`Does the shortest edge have to be on the MST? ` Yes. Consider Kruskal's
algorithm, which will always add the shortest edge first.

`Can the longest edge belong to the MST?` Yes, if the original graph already is
...
`Does a min-weight edge on every cycle have to belong to the MST?` No, because
