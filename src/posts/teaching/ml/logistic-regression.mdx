...
$$
\text{Simple Linear Regression: }h_0(x)=\theta_0+\theta_1x \\ \text{Sigmoid Function: }h_1(x)=\frac{1}{1+e^{-x}} = \frac{e^x}{e^x+1} \\ \text{Combined Logistic Function: } h_1 \circ h_0(x)=\frac{1}{1+e^{-\theta_0 - \theta_1x}}
$$
